...
export default function Hero() {
  return (
    ...
    >
      ...
      <div className="relative mx-auto grid w-full max-w-7xl grid-cols-1 items-center gap-16 px-6 py-32 lg:grid-cols-2 lg:gap-12 lg:px-12">
        <motion.div
          variants={container}
          initial="hidden"
          animate="show"
        ...
        >
          <motion.h1
            variants={item}
            className="font-display text-4xl font-black uppercase leading-[0.94] tracking-tight text-ink sm:text-5xl lg:text-6xl xl:text-[4rem]"
          >
          ...
          <motion.p
            variants={item}
            className="mt-4 font-mono text-sm tracking-wide text-ink-faint"
          >
            {profile.stack.join('  •  ')}
          </motion.p>
          ...
          <motion.div variants={item} className="mt-10 flex flex-wrap gap-4">
            <motion.a
              ...
              whileTap={{ scale: 0.98 }}
              className="group inline-flex items-center gap-2 rounded-full bg-ink px-6 py-3 text-sm font-medium text-base transition-colors hover:bg-ink/90"
            >
              View Projects
              <ArrowRight
